...
describe('String', () => {
  test('should return a string', () => {
    ...
    expect(tree[0].expression.type).toBe('String');
    expect(tree[0].expression.value).toBe('mystring');
  });

  test('should allow non-ascii chars in string', () => {
    const tree = parse(`<cfdiv property='√↓►;)┴≡±¡╥-╝åD░║«Y╚á'></cfdiv>`);
    expect(tree[0].attributes[0].left.value).toBe('property');
  ...
  test('should allow /> in string', () => {
    const tree = parse(`"/>";`);
    expect(tree[0].expression.type).toBe('String');
    expect(tree[0].expression.value).toBe('/>');
  });
  ...
  test('should allow escaped double quotes in string', () => {
    ...
    expect(tree[0].expression.type).toBe('String');
    expect(tree[0].expression.value).toBe('test\\"something\\"');
  });
