...
//
// Notable deviations
// * ColdFusion is case-insensitive, added 'i' after literal tokens
// * added CF operators
// * addition of Tag and tagContext
...
// also see this for some ideas of statement/Expression difference
...
// Expression = String / CallExpression / MemberExpression / Identifier / Struct / Array / /*binaryExpression /*/ Ternary
...
NonEscapeCharacter
  = !(EscapeCharacter / LineTerminator) any { return text(); }
...
// taken directly from JavaScript example Grammar [2]
...
CatchToken      = "catch"i         !IdentifierPart
...
ExtendsToken    = "extends"i       !IdentifierPart
FalseToken      = "false"i / "no"i !IdentifierPart
FinallyToken    = "finally"i       !IdentifierPart
...
InstanceofToken = "instanceof"i    !IdentifierPart
...
RequiredToken   = "required"i      !IdentifierPart
...
FunctionDeclaration
  = a:AccessType? ws r:DataType? ws FunctionToken ws id:$Identifier? ws
...
FunctionParameter
  = req:RequiredToken? ws type:DataType? ws name:$Identifier? defaultVal:(ws eq ws e:Expression { return e; })?
...
MemberExpression
  = head:(
      ...
      / NewToken ws callee:MemberExpression ws args:Arguments {
...
NewExpression
  = MemberExpression
  / NewToken ws callee:NewExpression {
      return { type: "NewExpression", callee: callee, arguments: [] };
    }

CallExpression
  = head:(
      callee:MemberExpression ws args:Arguments {
...
Arguments
  = "(" ws args:(a:ArgumentList ws {return a;})? ")" {
...
PostfixExpression
  = argument:LeftHandSideExpression ws operator:PostfixOperator {
      return {
...
MultiplicativeExpression
  = head:UnaryExpression
    tail:(ws MultiplicativeOperator ws UnaryExpression)*
...
EqualityExpression
  = head:RelationalExpression
    tail:(ws EqualityOperator ws RelationalExpression)*
    { return buildBinaryExpression(head, tail); }

EqualityExpressionNoIn
  = head:RelationalExpressionNoIn
    tail:(ws EqualityOperator ws RelationalExpressionNoIn)*
    { return buildBinaryExpression(head, tail); }
...
LogicalORExpression
  = head:LogicalANDExpression
    tail:(ws LogicalOROperator ws LogicalANDExpression)*
    { return buildLogicalExpression(head, tail); }

LogicalORExpressionNoIn
  = head:LogicalANDExpressionNoIn
    tail:(ws LogicalOROperator ws LogicalANDExpressionNoIn)*
    { return buildLogicalExpression(head, tail); }
...
AssignmentExpression
  = left:LeftHandSideExpression ws
    "=" !"=" ws
  ...
  / left:LeftHandSideExpression ws
    operator:AssignmentOperator ws
    right:AssignmentExpression
    {
      return {
        type: "AssignmentExpression",
...
// STATEMENT
// ===================
// will need to include all the wacky syntaxes that CF has for cfscript blocks
...
Block
  = "{" ws body:(s:StatementList ws {return s;})? "}" {
      return {
        type: "BlockStatement",
        body: optionalList(body)
      };
...
StatementList
  = head:Statement tail:(ws s:Statement {return s;})* { return buildList(head, tail); }

VariableStatement
  = VarToken ws declarations:VariableDeclarationList EOS {
      return {
...
VariableDeclarationList
  = head:VariableDeclaration tail:(ws "," ws v:VariableDeclaration {return v;})* {
      return buildList(head, tail);
    }

VariableDeclarationListNoIn
  = head:VariableDeclarationNoIn tail:(ws "," ws v:VariableDeclarationNoIn {return v;})* {
      return buildList(head, tail);
    }

VariableDeclaration
  = id:Identifier init:(ws i:Initialiser {return i;})? {
      return {
...
VariableDeclarationNoIn
  = id:Identifier init:(ws i:InitialiserNoIn {return i;})? {
      return {
        type: "VariableDeclarator",
        id: id,
        init: init
...
Initialiser
  = "=" !"=" ws expression:AssignmentExpression { return expression; }

InitialiserNoIn
  = "=" !"=" ws expression:AssignmentExpressionNoIn { return expression; }
...
IfStatement
  = IfToken ws "(" ws test:Expression ws ")" ws
    consequent:Statement ws
    ElseToken ws
    alternate:Statement
    {
      return {
...
IterationStatement
  = DoToken ws
    body:Statement ws
    WhileToken ws "(" ws test:Expression ws ")" EOS
    { return { type: "DoWhileStatement", body: body, test: test }; }
  / WhileToken ws "(" ws test:Expression ws ")" ws
    body:Statement
    { return { type: "WhileStatement", test: test, body: body }; }
  / ForToken ws
    "(" ws
    init:(e:ExpressionNoIn ws {return e;})? ";" ws
    test:(e:Expression ws {return e;})? ";" ws
    update:(e:Expression ws {return e;})?
    ")" ws
    body:Statement
    {
      return {
  ...
  / ForToken ws
    "(" ws
    VarToken ws declarations:VariableDeclarationListNoIn ws ";" ws
    test:(e:Expression ws {return e;})? ";" ws
    update:(e:Expression ws {return e;})?
    ")" ws
    body:Statement
  ...
  / ForToken ws
    "(" ws
    VarToken ws declarations:VariableDeclarationListNoIn ws
    InToken ws
    right:Expression ws
    ")" ws
    body:Statement
...
ContinueStatement
  ...
  / ContinueToken ws label:Identifier EOS {
...
BreakStatement
  ...
  / BreakToken ws label:Identifier EOS {
...
ReturnStatement
  ...
  / ReturnToken ws argument:Expression EOS {
      return { type: "ReturnStatement", argument: argument };
    }

WithStatement
  = WithToken ws "(" ws object:Expression ws ")" ws
    body:Statement
    { return { type: "WithStatement", object: object, body: body }; }

SwitchStatement
  = SwitchToken ws "(" ws discriminant:Expression ws ")" ws
    cases:CaseBlock
    {
      return {
...
CaseBlock
  = "{" ws clauses:(c:CaseClauses ws {return c;})? "}" {
      return optionalList(clauses);
    }
  / "{" ws
    before:(c:CaseClauses ws {return c;})?
    default_:DefaultClause ws
    after:(c:CaseClauses ws {return c;})? "}"
    {
...
CaseClauses
  = head:CaseClause tail:(ws c:CaseClause {return c;})* { return buildList(head, tail); }

CaseClause
  = CaseToken ws test:Expression ws ":" consequent:(ws s:StatementList {return s;})? {
      return {
        type: "SwitchCase",
        test: test,
...
DefaultClause
  = DefaultToken ws ":" consequent:(ws s:StatementList {return s;})? {
      return {
        type: "SwitchCase",
        test: null,
        consequent: optionalList(consequent)
...
LabelledStatement
  = label:Identifier ws ":" ws body:Statement {
      return { type: "LabeledStatement", label: label, body: body };
    }

ThrowStatement
  = ThrowToken ws argument:Expression EOS {
      return { type: "ThrowStatement", argument: argument };
    }

TryStatement
  = TryToken ws block:Block ws handler:Catch? ws finalizer:Finally? {
      return {
        type: "TryStatement",
        block: block,
        handler: handler,
  ...
  // I don't see the purpose of these -> made handler and finalizer optional above
  // / TryToken ws block:Block ws handler:Catch {
  //     return {
  //       type: "TryStatement",
  //       block: block,
  //       handler: handler,
  ...
  //     };
  //   }
  // / TryToken ws block:Block ws finalizer:Finally {
  //     return {
  //       type: "TryStatement",
  //       block: block,
  //       handler: null,
...
Catch
  = CatchToken ws "(" ws param:Identifier ws ")" ws body:Block {
      return {
        type: "CatchClause",
        param: param,
        body: body
...
Finally
  = FinallyToken ws block:Block { return block; }
